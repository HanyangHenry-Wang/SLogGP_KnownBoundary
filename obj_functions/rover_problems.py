...
# def l2cost(x, point):
#         return 0.5*np.linalg.norm(x - point, 1)+0.25*np.linalg.norm(x - point, 2)**2 #2. * np.linalg.norm(x - point, 1) + np.linalg.norm(x - point, 2)**2
...
#         # center = np.array([[1., 1.], [1.8, 0.0],[-0.25, 2.1],[-0.5,-0.5],[2.5,2.5],[-0.5,0.5],[1.,2.5]])
#         # sigma = np.array([0.5,0.6,0.45,0.35,0.3,0.25,0.3]) 
...
def l2cost(x, point):
        return 1. * np.linalg.norm(x - point, 2)
...
def Rover_8D(X,seed=1234):
        ...
        sigma = np.array([0.04]*113).reshape(-1,1) 

        
        cost_fn1 = GMCost(center, sigma) 
        ...
        costs = [cost_fn1, ConstObstacleCost(obstacles, cost=5.)]
        cost_fn = AdditiveCosts(costs)
...
class rover8:
    ...
    def __init__(self,negate=False):
        ...
        self.bounds = torch.tensor(np.array([[0.,1.]*8]).reshape(8,2).T)
